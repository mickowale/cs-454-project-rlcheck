...
class RNN_LSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("________ forward")
        h0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(device)
        c0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(device)

        # Forward propagate LSTM
        x = x.unsqueeze(0)
        # print("inside forward", x)
        out, _ = self.lstm(
            x, (h0, c0)
...
def optimize_model():
    ...
    for trans in transitions:   
        ...
        p = policy_net(state)[0,idx_action]
...
policy_net = RNN_LSTM(input_size, hidden_size, num_layers, num_classes).to(device)
target_net = RNN_LSTM(input_size, hidden_size, num_layers, num_classes).to(device)
